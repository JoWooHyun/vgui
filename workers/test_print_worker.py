...
class PrintJob:
    ...
    blade_speed2: int = 1200     # mm/min (구간2: boundary→140)
...
class TestPrintWorker(QThread):
    ...
    def _run_print_sequence(self):
        ...
        print(f"  - 블레이드 속도2: {job.blade_speed2} mm/min ({job.blade_boundary}→140mm)")
    ...
    def _process_layer(self, layer_idx: int, job: PrintJob) -> bool:
        ...
        if not self._motor_x_move(job.blade_boundary, job.blade_speed):
            ...
            self._is_stopped = True
            return False
        if not self._motor_x_move(140, job.blade_speed2):
