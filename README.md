...
| X축 | 블레이드 수평 이동 (레진 평탄화) | 0~140mm | 리드스크류 |
...
│   └── test_print_worker.py    # 테스트 모드 워커 (LED 없이 모터만)
├── windows/                    # 추가 윈도우
...
│   ├── test_material_page.py   # 15: 테스트 모드 소재 프리셋
│   └── print_test_page.py      # 16: 테스트 프린트 (설정+진행)
├── styles/                     # 스타일 정의
...
1. **초기화**: Z홈 → Z offset 이동 → X홈
2. **프라이밍 체크**: Setting에서 저장된 Y축 프라이밍 위치 확인 (미설정 시 차단)
3. **초기 레진 토출**: 프라이밍 완료 시 레진 1회 토출 + settle time 대기
4. **레벨링**: X축 블레이드 blade_start→blade_end (oneway, initial_leveling ON 시)
5. **레이어 루프**:
   - Z축 → 레이어 높이 이동
   - Y축 → 3단계 레진 토출 (Push-Pull-Return):
     - Push: -방향 토출 (position ≤ 0이면 즉시 레진 부족 감지)
     - Delay: dispense_delay초 대기
     - Pull: +방향 되돌리기 (distance > 0이면, 속도 = dist/delay)
     - Return: -방향 다시 밀기 (distance > 0이면, 속도 = dist/delay)
   - X축 → 블레이드 스윕 (blade_start→blade_end, 2구간 속도)
   - LED ON → 노광 대기 → LED OFF
   - Z축 → 리프트 (+8mm)
   - X축 → 복귀 (blade_end→0mm, 3000mm/min)
6. **완료/에러**: LED OFF → 사용자 선택 (GUI홈 / Z축홈)
...
| 버전 | 날짜 | 주요 변경 |
|------|------|----------|
| **v5.4** | 2026-05-29 | blade_start/blade_end 프리셋 이동, 기본값/범위 일괄 변경, Print Progress `:g` 포맷, LED Delay 설정 (1~60초), worker Z-lift/X복귀 동기화 |
| **v5.3** | 2026-05-28~29 | Material 2열 레이아웃, Leveling ON/OFF 토글, File Preview/Print Progress 설정 전체 표시, NumericKeypad 자동 클리어, 예상 시간 정확도 개선 |
| **v5.2** | 2026-05-27 | 테스트 모드 (TestPrintWorker, TestMaterialPreset, TestMaterialPage, PrintTestPage), Z offset, settle time, initial leveling, 블레이드 130mm |
| **v5.1** | 2026-05-26 | 블레이드 범위 설정 (blade_start/end), 블레이드 홈 복귀, 주사기 리필 프라이밍 UI, Y좌표 리셋 |
